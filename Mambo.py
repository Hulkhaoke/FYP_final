...
class Mambo:
    ...
    def _ack_packet(self, packet_id):
        ...
        packet = struct.pack("<BBB", self.data_types['ACK'], self.characteristic_send_counter['ACK_COMMAND'],
                             packet_id)
        self._debug_print(
            "sending packet %d %d %d" % (self.data_types['ACK'], self.characteristic_send_counter['ACK_COMMAND'],
                                         packet_id), 1)
    ...
    def _send_noparam_command_packet_ack(self, command_tuple):
        ...
        :return: True if the command was sent and False otherwise
        """
        self.characteristic_send_counter['SEND_WITH_ACK'] = (self.characteristic_send_counter[
                                                                 'SEND_WITH_ACK'] + 1) % 256
        packet = struct.pack("<BBBBBB", self.data_types['DATA_WITH_ACK'],
    ...
    def _send_enum_command_packet_ack(self, command_tuple, enum_value, usb_id=None):
        ...
        if (usb_id is None):
            ...
            packet = struct.pack("<BBBBBBBI", self.data_types['DATA_WITH_ACK'],
                                 self.characteristic_send_counter['SEND_WITH_ACK'],
                                 command_tuple[0], command_tuple[1], command_tuple[2], 0,
                                 usb_id, enum_value)
        return self._send_command_packet_ack(packet)
    ...
    def fly_direct(self, roll, pitch, yaw, vertical_movement, duration):
        ...
        while (time.time() - start_time < duration):
            self.characteristic_send_counter['SEND_NO_ACK'] = (self.characteristic_send_counter[
                                                                   'SEND_NO_ACK'] + 1) % 256
